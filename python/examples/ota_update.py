...
carry the version (tc-gu-01-slave-1.2.9.bin). Prefer the role selectors below:
...
def _default_firmware_for_role(role: str) -> str:
    ...
    The filename carries the version (tc-gu-01-slave-1.2.9.bin), so it cannot be
...
def _identify_image(fw_bytes: bytes):
    ...
    **按内容认,不按文件名认。** 发布镜像的文件名带版本
    (tc-gu-01-slave-1.2.9.bin),但文件名是可以被改的 —— 拷走、重命名、从别处
